...
def to_excel(area_a, area_i, blocks_a, blocks_i, raw):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        wb = writer.book
        base = wb.add_format({"font_name": "Meiryo UI"})
        ...
        for title, df in [("申込", area_a), ("発行", area_i)]:
            ...
            df.columns = ["total"] + [
                c.strftime("%Y/%m/%d") for c in df.columns[1:]
            ]
...
area_a = ra.pivot_table(index="領域", columns="日付", values="実績", aggfunc="sum", fill_value=0)
area_i = ri.pivot_table(index="領域", columns="日付", values="実績", aggfunc="sum", fill_value=0)
