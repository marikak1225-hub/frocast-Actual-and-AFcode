...
# 共通
def normalize(val):if pd.isna(val):return ""
    return (str(val).replace(" ", "").replace("　", "").replace("\n", "").replace("\r", "").strip())

def convert_date(val):
    try:s = str(int(val)) return pd.Timestamp(int(s[:4]), int(s[4:6]), int(s[6:8]))
    except:return pd.NaT

# マスター
def read_af_master(path):df = pd.read_excel(path, header=None, engine="openpyxl")header = None
    for i in range(len(df)):row = df.iloc[i].astype(str).apply(normalize)
    if row.str.contains("AFコード|AFｺｰﾄﾞ|ＡＦコード").any():header = i break
    if header is None:st.error("AFコード列が見つかりません")st.stop()
...
def process_raw(df_raw, af_master, start, end, kind):
    ...
    records = []
    for _, r in df_raw.iterrows():dt = r["日付"]
    if pd.isna(dt) or not (start <= dt <= end):continue
    for col in df_raw.columns[1:]:val = r[col]
    if pd.isna(val) or val == 0:continue
    info = af_map.get(normalize(col))
    if info is None:continue
    records.append([kind, dt, info["割り振り"], info["領域"], val])
    return pd.DataFrame(records,columns=["種別", "日付", "割り振り", "領域", "実績"],)

# 割り振りブロック
def create_blocks(df, target_master):act = df.pivot_table(index="日付", columns="割り振り", values="実績", aggfunc="sum", fill_value=0)tar = act.copy()
    for d in act.index:
    for c in act.columns:tar.loc[d, c] = get_target(target_master, d, c)
    for t in [act, tar]:t["total"] = t.sum(axis=1)
    gap = act - tar
    ratio = act.divide(tar).replace([float("inf"), -float("inf")], pd.NA)
    for df_ in [act, tar, gap, ratio]:df_.index = df_.index.strftime("%Y/%m/%d")return act, tar, gap, ratio

# Excel出力
def to_excel(area_apply, area_issue, blocks_apply, blocks_issue, raw_df):output = BytesIO() with pd.ExcelWriter(output, engine="xlsxwriter") as writer:

# 領域別
ws_area = writer.book.add_worksheet("領域別") writer.sheets["領域別"] = ws_area row = 0
        for title, df in [("申込", area_apply), ("発行", area_issue)]:ws_area.write(row, 0, title)
        df.to_excel(writer, sheet_name="領域別", startrow=row + 1)row += len(df) + 4

# 割り振り別
ws_assign = writer.book.add_worksheet("割り振り別") writer.sheets["割り振り別"] = ws_assign row = 0
        for label, blocks in [("申込", blocks_apply), ("発行", blocks_issue)]:ws_assign.write(row, 0, label) row += 1
        for title, df in zip(["■実績", "■目標", "■GAP", "■Target vs Actual"], blocks):ws_assign.write(row, 0, title)
        df.to_excel(writer, sheet_name="割り振り別", startrow=row + 1) row += len(df) + 3 row += 2

# ローデータ
raw_df.to_excel(writer, sheet_name="日別_集計ローデータ", index=False) return output.getvalue()
...
if not apply_file or not issue_file:st.stop()
...
target_apply = read_target(TARGET_APPLY_PATH) target_issue = read_target(TARGET_ISSUE_PATH)
df_a = process_raw(df_apply, af_master, start, end, "申込") df_i = process_raw(df_issue, af_master, start, end, "発行")
